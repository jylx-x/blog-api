  ...
  return (
    <div className="p-12 flex-1 bg-slate-50">
      ...
      <div className="grid lg:grid-cols-4">
        {latest.map((posts) => (
          <div className="p-4 bg-gray-600 rounded m-4 shadow-md text-white" key={posts._id}>
            <h2 className="text-3xl font-bold truncate">{posts.title}</h2>
            <p className="line-clamp-4 h-24 mt-4">{posts.body}</p>
            <div className="my-3">
              <a href={"/posts/" + posts._id} className="underline underline-offset-1 text-sm">Read More</a>
            </div>
